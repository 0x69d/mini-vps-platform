...
- **qemu-guest-agent**: domain XML に guest-agent 用 virtio チャネル
  (`org.qemu.guest_agent.0`)が常設され、cloud-init の `packages` で
  `qemu-guest-agent` を全 VM に導入する(Fedora 系のように同梱済みのイメージでは
  冪等な no-op)。VM の IP は agent 優先(`SRC_AGENT`)・DHCP リースフォールバック
  (`SRC_LEASE`)で観測するため、agent が動かないゲストは libvirt 管理ネットワーク
  (`default`)ではリースで IP が取れるが、OVS セグメント(`seg1`〜`seg3`)では
  `status` の `ip` が確定しない。
...
- qemu-guest-agent を導入できないゲスト(パッケージ未提供・ネットワーク不通で
  cloud-init の `packages` が失敗する等)は、OVS セグメント上では `status` の
  `ip` が確定しない(VM 自体は動作する)。
