...
`startup_script` を再度効かせたい場合は、そのたびに `--startup-param`/`secrets`
を渡し直す必要がある(自動的には引き継がれない)。また `create` の冪等判定は
spec のみを比較するため、既存 VM と spec が完全一致する `create` 呼び出しは
secrets を渡しても no-op のままで startup_script は再実行されない
(cloud-init が初回起動時のみ実行されるため実用上問題ない)。
...
変更されることがある(実際に `Qwen3-Coder` 系は2026年7月21日に提供終了予定と
アナウンスされている)。最新のモデル一覧は `GET https://api.ai.sakura.ad.jp/v1/models`
で確認すること。

## トラブルシューティング

実機検証済みの確認手順(対話的な SSH ログインで行うこと。理由は下記の
「非対話シェルでは動かない」を参照):
...
**非対話シェルでは動かない**: `ssh host 'コマンド'` のように非対話でコマンドを渡すと、
Ubuntu の既定 `~/.bashrc` は冒頭のガード(`case $- in *i*) ;; *) return;; esac`)で
早期リターンし、末尾に追記した `PATH`(OpenCode 本体)と `AI_ENGINE_TOKEN` の
読み込みが実行されない。対話的にログインしてシェルプロンプトから操作するか、
非対話実行したい場合は `export PATH=$HOME/.opencode/bin:$PATH` と
`. ~/.config/opencode/ai-engine-token.env` を明示的に実行すること。

OpenCode のカスタム OpenAI 互換プロバイダ設定は、`apiKey` に `{env:VAR}` 構文
(`${env:VAR}` ではない)を使う必要がある。この構成(`@ai-sdk/openai-compatible` +
カスタム `baseURL` + `{env:VAR}` の apiKey)は実機で正常動作を確認済み(ツール呼び出し
によるファイル書き込みまで含めて動作した)。
