...
以下の3つの環境変数がすべて設定されているときのみ有効になる。1つでも欠ければ
機能全体が完全に無効になり、nsupdate は一切呼ばれない。
...
「TSIG鍵の初期化」を参照。鍵はパス参照のみで、mini-vps-platform は鍵ファイルを
開かない。鍵の中身は spec・libvirt metadata・ログ・例外メッセージのいずれにも
現れない。ホスト側には `nsupdate`(`bind9-dnsutils`パッケージ)が必要。

## 動作

- `create()` 成功後(新規作成時のみ): VM の最初の静的NICの IP で
...
- 登録は `update delete` → `update add` の組で書くため冪等。
  A(正引きゾーン)と PTR(in-addr.arpa)は別ゾーンのため、RFC 2136(動的更新は
  1メッセージ=1ゾーン)に従い `send` を2回に分ける。
- タイムアウト: `nsupdate -t 5` + subprocess の 15 秒の二段構え。dns-1 が
  無応答でも create/delete が長時間ブロックしない。
...
というレイヤの逆転を含む。通常この向きの依存は避けるべきだが、以下の2点で
無害化している:
...
再作成する操作も失敗するという循環依存のデッドロックに陥る。制御プレーンの
...
- A 成功・PTR 失敗のような部分失敗がありうる(send を2分割するため)。
...
- `update delete <name> A` は同名のすべての A レコードを消す。同名に手動で
