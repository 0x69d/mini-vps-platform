...
| `network` | str（`name` と同じ文字種制約）。Ansible で事前定義済みのネットワーク名(`default`・`seg-a`・`seg-b`)を指定する。未定義名を指定すると作成時に libvirt エラーになる | 任意 | `default` |
...
(定義は `ansible/vars/network_segments.yml`)。

| name | bridge | サブネット | DHCP レンジ |
|---|---|---|---|
| `default` | virbr0 | 192.168.122.0/24 | .2〜.254 |
| `seg-a` | virbr-sega | 192.168.124.0/24 | .2〜.254 |
| `seg-b` | virbr-segb | 192.168.125.0/24 | .2〜.254 |
...
グループ追加・default ネットワーク・セグメント NAT ネットワーク(`seg-a`/`seg-b`)・
