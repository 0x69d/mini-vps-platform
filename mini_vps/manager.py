...
def _write_spec(dom, spec: dict, live: bool = False) -> None:
    """VM スペックを YAML 化し、dom の <metadata> に書き込む。

    ElementTree でテキストノードを組むことで、spec 値の & < > が自動エスケープされる。
    既定の flags は AFFECT_CONFIG のみ。新規作成では起動前に書くため、起動時の live が
    CONFIG を引き継ぐ。稼働中の domain へ差分を収束させたときは live=True で live 側
    にも書く。_read_spec(flags=0 は稼働中なら live を読む)が古い spec を返し続け、
    次の create() が同じ差分を何度も収束させるのを防ぐため。

    Args:
        dom: 書き込み先の domain。
        spec: 書き込む VM スペック。
        live: 稼働中の domain の live 定義にも書くか。
    """
    el = ET.Element("spec")
